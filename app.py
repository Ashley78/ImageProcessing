...
def make_grayscale(file):
    image_array= np.fromstring(file,dtype="uint8")
    print("Image array : ",image_array)
    
    decode_array_toimg= cv2.imdecode(image_array,cv2.IMREAD_UNCHANGED)
    print("Decoded image : ",decode_array_toimg)

    converted_gray_img= cv2.cvtColor(decode_array_toimg,cv2.COLOR_RGB2GRAY)
